...
FIELD_LABELS = {
    ...
    'l10n_in_relationship': 'Emergency Relationship', 'emergency_phone': 'Emergency Phone',
    ...
    'spouse_visa_no': 'Spouse Visa No', 'spouse_visa_expire_date': 'Spouse Visa Expiry Date',
...
class HrProfileChangeRequest(models.Model):
    ...
    def action_submit(self):
        ...
        self.employee_id.sudo().write({
            ...
            'last_notification_type':    'pending',
            'last_notification_message': 'Your profile change request is awaiting HR review.',
            'last_notification_reason':  False,
            'last_notification_ref':     self.name,
        })
    ...
    def action_approve(self):
        ...
        self._add_trail(action='approved', note=f'Approved by {self.env.user.name}. {len(write_vals)} field(s) written to employee record.')
        self._send_mail_to_employee('approved')

        # ── Clear overlay + WRITE approved notification (DO NOT clear notification) ──
        self.employee_id.sudo().write({
            'last_portal_submission':    False,
            'last_submission_state':     False,
            'last_notification_type':    'approved',
            'last_notification_message': (
                f'Your profile update request {self.name} has been approved by HR. '
                f'Your record has been updated successfully.'
            ),
            'last_notification_reason':  False,
            'last_notification_ref':     self.name,
        })
    ...
    def _send_mail_to_hr(self):
        try:
            ...
            mail = self.env['mail.mail'].sudo().create({
                ...
                'auto_delete': False,
                'body_html': f'<div style="font-family:Arial,sans-serif;max-width:600px;"><div style="background:#4e73df;padding:20px;"><h2 style="color:white;margin:0;">New Profile Change Request</h2></div><div style="padding:20px;background:#f9f9f9;"><p>Dear {hr_person.name},</p><p><b>{self.employee_id.name}</b> has submitted a profile update request.</p><p>Reference: <b>{self.name}</b></p><p>Please login to Odoo → Profile Change Requests to review.</p></div></div>',
            })
            mail.sudo().send()
        except Exception as e:
