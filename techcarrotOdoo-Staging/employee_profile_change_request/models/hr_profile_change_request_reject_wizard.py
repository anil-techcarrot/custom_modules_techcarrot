...
class HrProfileChangeRequestRejectWizard(models.TransientModel):
    ...
    request_id = fields.Many2one(comodel_name='hr.profile.change.request', string='Request Reference', required=True, readonly=True)
    employee_name = fields.Char(string='Employee', related='request_id.employee_id.name', readonly=True)
    ...
    def action_confirm_reject(self):
        ...
        # ── Keep overlay for resubmit + WRITE rejected notification ──
        req.employee_id.sudo().write({
            'last_submission_state':     'rejected',
            'last_portal_submission':    req.submitted_data,
            'last_notification_type':    'rejected',
            'last_notification_message': 'Your profile update request was rejected by HR.',
            'last_notification_reason':  self.rejection_reason.strip(),
            'last_notification_ref':     req.name,
        })

        if hasattr(req, '_add_trail'):
            req._add_trail(action='rejected', note='Rejected by %s' % self.env.user.name, reason=self.rejection_reason.strip())
