...
class EmployeePortalProfileSubmit(http.Controller):
    ...
    @http.route('/my/employee/personal', type='http', auth='user', website=True, methods=['GET', 'POST'], csrf=False)
    def portal_employee_personal(self, **post):
        ...
        # ── portal_overlay: only show for pending/rejected, NOT approved ──
        portal_overlay = {}
        ...
        # ── Build notification from last_submission_state ──────────────
        notification = None
        state = employee.last_submission_state

        if state == 'approved':
            # Find the latest approved request for the reference number
    ...
    def _handle_post(self, employee, post):
        try:
            ...
            if not submitted:
                return request.make_json_response({'success': False, 'error': 'No data was submitted.'})
            ...
            _logger.info('Profile change request %s created for employee %s — %d field(s) changed', req.name, employee.name, len(changed))

            return request.make_json_response({
                'success':   True,
                'reference': req.name,
                'message':   'Your profile update request has been submitted. HR will review and notify you by email.',
            })

        except Exception as e:
            _logger.error('Error creating profile change request for %s: %s', employee.name, str(e))
